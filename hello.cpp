...
class TreeShapeListener : public RedoParserBaseListener {
private:
	string comma=",";
	string period=".";
	string change_date;
        string layer_opcode_string;
        string data_object_id_string ;
        string layer_string;
        string opcode_string ;
        string invalid_string="N";
        string media_recovery_marker_string="N";
	string opcode_lookup[50][150];
public:
  
  void enterChg_obj(RedoParser::Chg_objContext * ctx) override { }
  void exitChg_obj(RedoParser::Chg_objContext * ctx) override {
        data_object_id_string = ctx -> obj_value()->getText();
  }
     
   void exitChange_records(RedoParser::Change_recordsContext * ctx) override {
           stringstream layer_id(layer_string);
	   stringstream opcode_id(opcode_string);
//	   string opcode_desc = opcode_lookup[layer_id][opcode_id];
	   cout << data_object_id_string + comma + layer_string +period + opcode_string + comma + change_date  ;
   }

   void enterLayer_opcode(RedoParser::Layer_opcodeContext * ctx) override { }
   
   void exitLayer_opcode(RedoParser::Layer_opcodeContext * ctx) override {

       layer_string = ctx->layer()->getText();
       opcode_string =  ctx->opcode()->getText(); 
   }

   void exitRedo_record(RedoParser::Redo_recordContext * ctx ) override {

	   change_date=ctx->date_value()->getText()  + "\n" ;

  
   }
   void enterRedo_file(RedoParser::Redo_fileContext * /*ctx*/) override {
...
int main(int argc, char *argv[]) {
	string line;
	ifstream RedoFile (argv[1]);
	if (RedoFile.is_open()) {
		ANTLRInputStream input(RedoFile);
...
//			std::cout << token->toString() << std::endl;
		...
		tree::ParseTree *tree = parser.redo_file();
